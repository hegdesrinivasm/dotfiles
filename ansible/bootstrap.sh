...
main() {
    # Ensure pip --user bin is on PATH (macOS). This must run before any
    # ansible-* commands, and unconditionally, because install_ansible() is
    # skipped when ansible-playbook is already found.
    if [[ "${os}" == "Darwin" ]]; then
